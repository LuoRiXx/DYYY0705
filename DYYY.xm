...
- (void)didEnterRoom:(id)room {
    ...
    [[DYYYLiveLuckyBagManager sharedManager] handleLiveRoomEntered:room ?: self audienceViewController:self];
}
...
- (void)didCloseRoom:(id)room closeType:(unsigned long long)type {
    ...
    [[DYYYLiveLuckyBagManager sharedManager] handleLiveRoomClosed:room ?: self];
    %orig;
}
...
- (void)dealloc {
    ...
    [[DYYYLiveLuckyBagManager sharedManager] handleLiveRoomClosed:self];
    %orig;
}
%end
...
- (void)layoutSubviews {
    %orig;
    [[DYYYLiveLuckyBagManager sharedManager] handleLuckyBagViewAppeared:self];
...
- (void)sendAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    [[DYYYLiveLuckyBagManager sharedManager] handleControlAction:action target:target sender:self event:event];
    %orig;
}
%end
...
- (void)setState:(UIGestureRecognizerState)state {
    ...
    if (state == UIGestureRecognizerStateEnded) {
        [[DYYYLiveLuckyBagManager sharedManager] handleGestureRecognizer:self];
    }
}
%end
...
- (void)setText:(NSString *)text {
    %orig(text);
    [[DYYYLiveLuckyBagManager sharedManager] handleTextInputView:self text:text];
}

- (void)insertText:(NSString *)text {
    %orig(text);
    [[DYYYLiveLuckyBagManager sharedManager] handleTextInputView:self text:self.text];
}
